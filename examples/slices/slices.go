...
func main() {
    ...
    //builtin `make`. Ici, on crée une slice de `string`s
    // de longueur `3` (initiallement de valeurs nulles).
    s := make([]string, 3)
    fmt.Println("emp:", s)
    ...
    //  nouvelle slice est renvoyée par append : la slice
    ...
    // syntaxe `slice[debut:fin]`. Par exemple, l'exemple
    ...
    // Les slices peuvent être composées en structures de
